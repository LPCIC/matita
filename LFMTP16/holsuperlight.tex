...
of context variables visible to \verb+K+, i.e. introduced by
the \verb+pi+ binder in the \verb+lam+ rule for the \verb+term+
predicate.
...
The HOL library we have now is not large enough yet to compare the speed of HOL Light with our own implementation. Currently, it includes only the definition of logical constants and their properties, the Knaster-Tarski fixpoint theorem, and a manual construction of natural numbers from a constructive universe. It ends with the definition of addition between natural numbers and the proof that it is commutative. Before progressing in the library, we first need to 1) implement a transfer package to automatize some of the proofs, like we did for monotonicity proofs, that are completely automatized; 2) automatize the construction of inductive types via the inductive definition package (that we already have) and the transfer package; 3) improve proof automation and revise the set of tactics.

LA FRASE SEGUENTE ORA E' FALSA PER VIA DEL TYPE-CHECKING RIPETUTO

In the meantime, the impression is that we could be slower than HOL Light by one or two order of magnitudes, and the hope is that one order of magnitude could eventually disappear moving to a compiler for $\lambda$Prolog.
