...
type instruction = Ins_put_variable_t of inscatRRX
  ...
  | Ins_execute_link_only of inscatCWPX
  | Ins_call_link_only of inscatI1CWPX
  | Ins_put_variable_te of inscatRRX
...
let getSize_execute_link_only = inscatCWPX_LEN
let getSize_call_link_only = inscatI1CWPX_LEN
let getSize_put_variable_te = inscatRRX_LEN
...
let writeInstruction inst =
  ...
  | Ins_execute_link_only(arg) -> writeopcode 142; writeCWPX arg
  | Ins_call_link_only(arg) -> writeopcode 143; writeI1CWPX arg
  | Ins_put_variable_te(arg) -> writeopcode 144; writeRRX arg
...
let readInstruction getKindFunc getConstantFunc =                             
  ...
  else  if opcode = 141 then (Ins_create_type_variable (readEX ()), inscatEX_LEN)
  else  if opcode = 142 then (Ins_execute_link_only (readCWPX ()), inscatCWPX_LEN)
  else  if opcode = 143 then (Ins_call_link_only (readI1CWPX ()), inscatI1CWPX_LEN)
  else (Ins_put_variable_te (readRRX ()), inscatRRX_LEN)
