...
outside it. The alternating performance \tedius{} on the reduction tests have
to be attributed to the $\lambda\sigma$ machinery
(see Section~\ref{sec:dbl}) employed to cross binders:
by their very nature, explicit
substitutions fit well a lazy reduction strategy like cbn (even if their space
overhead is visible).  On the contrary they are counterproductive in a strict
reduction strategy like cbv that, traversing the redex arguments eagerly ends
up systematically pushing the suspended substitution to the leaves of the
term, i.e. completely defeating the purpose of the entire machinery.  
If one makes \tedius{} artificially
push explicit substitutions in the cbn case too, he halves memory consumption
but degrades the performances of approximatively 10 seconds.  Our conclusion
is that our approach, requiring no substitution for crossing binders, makes
\elpi{} not only faster, but also more predictable performance wise: as one
