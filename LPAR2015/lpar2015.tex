...
We introduce \lp{} and discuss the role of $\beta$-reduction on
...
exploiting the forall quantifier, written \verb+pi x\G+, together with logical
...
In this first part of the example, the meta-level $\beta$-reduction is only employed
to inspect a term under a binder by replacing the bound name with a fresh
constant.  The second part (reduction)
...
one typically tracks the \emph{level} of unification variables and fresh constants.  The proof theoretic interpretation of a \lp{} execution as an intuitionistic proof gives the following reading: unification is taking place under
...
a level that corresponds to the length of the current ``context'', and whenever
...
will be able to completely avoid such substitution. \verb+T+, being at
level zero, cannot be instantiated with a term containing \verb+c+ nor
\verb+d+.
...
real names, but canonical ``names'' (hence $\alpha$-equivalence becomes syntactic equality).  De Bruijn introduced
...
\begin{description}
\item[DBL1] an occurrence keeps its level when moved under additional binders,
i.e. $x_i$ in $\Gamma$ remains $x_i$ in $\Gamma, \Delta$;
\item[DBL2]
when $\beta$-reduction occurs under a context the variables bound
in such context are not renamed in the reduct.
\end{description}
...
which they occur in the context.  From now on we subscript variables
...
Note that when $k$ is $0$ the variable is not applied.  A consequence of
...
\marginpar{FALSE, or better, subst makes copy exit the fragment.  Maybe we could say here that exp1 revised is in RFF, and use the same example later on to explain that one can exit the frament...}
...
implemented caching the maximum level in the term, and is something one
typically pre-computes on the input term in linear time.
...
\marginpar{what do we do with cbn? not sure my reading is ok?}
The table shows that \elpi{} shines on programs in \rff{}, and compares well
outside it.  The reduce\_cbn test uses, in \tedius{}, the explicit substitution
calculus to perform the substitution in a lazy way, that is exactly what it is
designed for.
...
environment that the OCaml code has to explicitly handle. 
...
or (bytecode) execution, while pre-processing accounts for parsing,
