...
In \elpi{} we chose DBL by observing the following two properties.
\begin{description}
\item[DBL$_1$] the name of a variable is context independent, i.e. $x$ is
...
The non-null superscript $j$ annotates unification variables with their
visibility scope.  A variable $X^j$ has visibility of all names strictly
...
\begin{mydef}[RFF]
A term is in the Reduction Free Fragment if and only if
every occurrence of a unification variable $X^j$ occurs applied to
...
showing that the fragment is already quite expressive.
...
\begin{myprop}[Constant time weak head $\beta$-reduction]
Let $\sigma$ be a \emph{valid} substitution for existentially quantified variables.  Then the weak head normal form of $(X^j~x_j \ldots x_{j+k-1}) \sigma$
can be computed in constant time.
\end{myprop}

A valid substitution assigns to $X^j$ a term $t$ of the right type (as in simply
type lambda calculus) and that has all free variable visible by $X^j$ (all
$x_i$ are such that $i < j$).  Let $X^j \sigma = \lambda x_j. \ldots \lambda x_{j+n}.t$. Then
...
In equation~\ref{deffrag} the equality is syntactical: no lifting
of $t$ is required. Therefore the $\beta$-reductions triggered
by the substitution of $X^j$ require constant time and no pass over the term.

\begin{myprop}[Constant time unification]
A unification problem of the form  $X^j~x_j\ldots x_{j+k-1} \equiv t$
admits a constant time unification.
\end{myprop}

The unification problem $X^j~x_j\ldots x_{j+k-1} \equiv t$ can always be
rewritten in $X^j \equiv \lambda x_j. \ldots \lambda x_{j+k-1}. Y^{j+k} \land
Y^{j+k} \equiv t$.  The former is a trivial assignment that requires no check.
The latter can be implemented in constant time when no occur-check is needed
...
(just one integer) and is something one can also  pre-compute on the input term
in linear time.  With such maximum level $l$ at hand the problem
$Y^{j+k} \equiv t$ can be decided in constant time by simply comparing $j+k$
with $l$.

These properties let us implement the introduction rule for the $\forall$
quantifier, i.e. the operational semantics of \verb+pi+ in constant time for
terms in the RFF.  A step by step example follows.

\paragraph{Backchaining clauses in DBL notation.}
\label{ssec:price}
...
& \verb+of (lam F+$^0$\verb+) (impl A+$^0$\verb+ B+$^0$\verb+) :- pi x+$_0$\verb+\ of x+$_0$\verb+ A+$^0$\verb+ => of (F+$^0$\verb+ x+$_0$\verb+) B+$^0$ \\\hline
\verb+ x+$_0$;\verb+(of x+$_0$\verb+ A+$^0$\verb+)+ & \verb+of (lam x+$_1$\verb+\app x+$_0$\verb+ x+$_1$\verb+) B+$^0$  \\
& \verb+of (lam G+$^1$\verb+) (impl C+$^1$\verb+ D+$^1$\verb+) :- pi x+$_1$\verb+\ of x+$_1$\verb+ C+$^1$\verb+ => of (G+$^1$\verb+ x+$_1$\verb+) D+$^1$\verb++  \\\hline
\verb+ x+$_0$;\verb+(of x+$_0$\verb+ A+$^0$\verb+)+ & \verb+of (app x+$_0$\verb+ x+$_1$\verb+) D+$^0$  \\
\verb+ x+$_1$;\verb+(of x+$_1$\verb+ C+$^0$\verb+)+ & \verb+of (app M+$^2$\verb+ N+$^2$\verb+) S+$^2$\verb+ :- of M+$^2$\verb+ (impl R+$^2$\verb+ S+$^2$\verb+), of N+$^2$\verb+ R+$^2$  \\\hline
\verb+ x+$_0$;\verb+(of x+$_0$\verb+ A+$^0$\verb+)+ & \verb+of x+$_0$\verb+ (impl R+$^2$\verb+ S+$^0$\verb+), of x+$_1$\verb+ R+$^2$  \\
\verb+ x+$_1$;\verb+(of x+$_1$\verb+ C+$^0$\verb+)+ &  \\\hline
\end{tabular}
\end{center}

After the first resolution step we obtain
\verb+F+$^0$\verb+:= x+$_0$\verb+\lam x+$_1$\verb+\app x+$_0$\verb+ x+$_1$
and \verb+T+$^0$\verb+:= impl A+$^0$\verb+ B+$^0$.  The extra clause
about \verb+x+$_0$ in the context and a new subgoal.
Note that the redex \verb+(F+$^0$\verb+ x+$_0$\verb+)+ is in the RFF and
thanks to equation~\ref{deffrag} weak head normalizes in constant time
...
variable, as happens in the unification problem
\verb+(B+$^0$\verb+=impl C+$^1$\verb+D+$^1$\verb+)+ that prunes
\verb+C+ and \verb+D+.

The choice of using DBL for bound variables has two important
effects, a good and a bad one.  First, clauses containing no bound variables, like
\verb+(of x+$_0$\verb+ A+$^0$\verb+)+ , requires no processing.
If one uses DBI...
The drawback is that clauses with bound variables, like the one for \verb+lam+,
need to be lifted: the first time the bound variable is named \verb+x+$_0$,
while the second time \verb+x+$_1$. This renaming, thanks again to property X of DBL
can be performed in constant time, using the very same machinery one uses to
refresh the unif variables.  E.g. in the WAM the unification involving the head
of a clause is always executed by assigning variable to fresh stack cells
(i.e. the clause is not refreshed, just the stack pointer is incremented).
One can represent the locally bound variable as an extra unification variable,
and initialize, when \verb+pi+ is crossed, the corresponding stack cell to
the first \verb+x+$_i$ no yet in the context.

\paragraph{Stability of the RFF.}
Unlike the pattern fragment, the RFF is not stable under \lp{} resolution.
The cause is that a clause that contains only terms in the RFF, because
of the lifting phenomenon in the previous paragraph may generate terms
outside the fragment.
Therefore, and contrarily to what happens with the pattern fragment, an
implementation must handle both terms in the RFF, with their efficient
...
remains in the fragment during computation, or they can be slightly modified to
