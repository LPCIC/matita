   ...
   match x with
    ...
    | Arg (i,args) when argsdepth >= to_ ->
        ...
        if a == dummy then begin
            let r = ref dummy in
            let v = UVar(r,to_,[]) in
(*NNN            if not last_call then trail := Stack_addr (e,i) :: !trail;*)
            e.(i) <- v;
...
(* TODO: check all places where RestrictionFailure should be captured,
   in patternunif.ml as well *)
     (try
       ...
       (*Format.eprintf "<- %a\n%!" (ppterm be) be.(i);*)
(*NNN       if not last_call then trail := Stack_addr (be,i) :: !trail;*)
       true
...
List.iter (fun (_,g) -> Format.eprintf "GS %a\n%!" ppterm g) gs;*)
    ...
    | c :: cs ->
        ...
        let c_env = Array.make c.vars dummy in
        match
...
*)
                let g' =
                 ...
                 to_heap g_depth last_call trail ~from:c.depth ~to_:g_depth
                  c_env g' in
                let gs' =
                 List.map
                  (fun x->
                    p, c_env, g_depth,
                     to_heap g_depth last_call trail ~from:c.depth ~to_:g_depth
                      c_env x) gs'
                in
                 run p g_depth c_env g_depth g' gs' next alts oldalts)
    in
      select cp
...
let rec stack_term_of_ast lvl l l' =
 function
  ...
  | AST.Const f -> 
     ...
     if ('A' <= c && c <= 'Z') || c = '_' then
      let v = snd (stack_var_of_ast l (Parser.ASTFuncS.pp f)) in
      l,l',v
     else
  ...
  | AST.App(AST.Const f,tl) ->
     ...
     if ('A' <= c && c <= 'Z') || c = '_' then
      ...
      let tl =
       ...
       in
        aux tl in
      (match stack_var_of_ast l (Parser.ASTFuncS.pp f) with
...
let impl =
 (module struct
  ...
  let msg (q_names,q_env,q) =
   Format.fprintf Format.str_formatter "Pattern unification only, almost lazy refresh: %a" (uppterm q_names q_env) q ;
